...
**Resolved 2026-09-17 (`/impeccable audit` fixes — harden, clarify, adapt, polish):** Ran `/impeccable audit`, scored 12/20 (Acceptable), then fixed every P1/P2 finding one command at a time. Harden: `--text-hero`/`--text-ui` were referenced in `AuthShell.tsx` but never defined in `globals.css`, silently breaking the sign-in/sign-up heading typography — added both tokens; `--brand-accent-text` was defined as `#FFFFFF` but never actually passed contrast against `--brand-accent` (`#FF6B6B`) in any theme (2.78:1, fails even the 3:1 large-text floor) — changed to `#5C1414` (4.83:1, passes AA) and pointed `CorrectionDisclosure`'s "Easy Fix" badge at it instead of the theme-flipping `--ink`. Clarify: `MicButton` (the app's core hold-to-talk control) had no accessible name — added `aria-label`; `TurnCard`'s speed buttons had no `aria-pressed`; `ConversationScreen`'s `StatusLine` (errors, offline notice, "Thinking…", text-scale limit message) had no live region — added `role="status" aria-live="polite"` once at the shared component instead of one-off per message. Adapt: rate-selector buttons, A-/A+ text-scale buttons, and the zh-only toggle were all under the 44×44px touch-target minimum — added `minWidth`/`minHeight: 44`. Optimize: skipped intentionally — the flagged `TurnCard` re-render cost is not a measured bottleneck at the 25-turn cap, and `optimize.md` itself warns against fixing what isn't measured to be slow. Verified: `tsc --noEmit`, `eslint`, and `vitest run` (139/139) all clean after every step; diff is 6 files, 18 insertions/3 deletions, no churn. Re-ran `/impeccable audit` after — see its own dated entry for the new score.
